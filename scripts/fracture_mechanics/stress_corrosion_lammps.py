...
stress_corrosion_atoms = parameter('stress_corrosion_atoms',"None")
if stress_corrosion_atoms == "None":
    stress_corrosion_atoms = None

stress_corrosion_timer = parameter('stress_corrosion_timer',3) # add after this many loops of crack arrest
...
stress_corrosion_time_val = stress_corrosion_timer
...
for knum,K in enumerate(kvals):
    ...
    while not at_steady_state:
        ...
        set_up_simulation_lammps(lmp,temp_path,mass,cmds,sim_tstep=sim_tstep,damping_strength_right=damping_strength_right,damping_strength_left=damping_strength_left
                                 ...
                                 y_fixed_length=y_fixed_length, weak_damp_thickness_ratio=weak_damp_thickness_ratio,
        ...
        if me == 0:
            ...
            if (i >= initial_damping_time) or (not initial_damp):
                ...
                #if the crack tip has not moved in the last 3 loops, then tick down stress corrosion timer
                crack_tip_sim_times = np.append(crack_tip_sim_times,sim_time)
                if len(crack_tip_positions) > 3:
                    if np.max(crack_tip_positions[-3:]) - np.min(crack_tip_positions[-3:]) < 1:
                        stress_corrosion_time_val -= 1
                        if stress_corrosion_time_val<0:
                            stress_corrosion_time_val = stress_corrosion_timer

            
            # --------------- add stress corrosion atoms --------------- #

            if stress_corrosion_atoms is not None:
                if stress_corrosion_timer == 0:
                    #add stress corrosion atoms 5 angstrom to the left of the crack tip and give them a rightwards velocity
                    if len(stress_corrosion_atoms) > 1:
                        stress_corr_pos = stress_corrosion_atoms.get_positions()
                        stress_corr_pos_mean = np.mean(stress_corr_pos,axis=0)
                    else:
                        stress_corr_pos_mean = stress_corrosion_atoms.position
                    
                    acc_strip_thickness = final_crack_state.get_cell()[2,2]

                    full_crack_tip_pos = [tip_pos-5, tip_pos_y, np.random.uniform(0,acc_strip_thickness)]
                    stress_corr_pos_mean += full_crack_tip_pos - stress_corr_pos_mean
                    stress_corrosion_atoms.set_positions(stress_corr_pos_mean)
                    stress_corrosion_atoms.set_velocities(np.array([0.5,0,0]))
                    final_crack_state += stress_corrosion_atoms
                    final_crack_state.wrap()
            ...
            new_slab = final_crack_state.copy()
