...
class TestBuild3DPeriodicFractureCell(matscipytest.MatSciPyTestCase):
    ...
    def build_3D_cluster(self,build_func,nzlayer,crack_surface,crack_front):
        ...
        A = np.zeros([3, 3])
        directions = [np.cross(crack_surface,crack_front),crack_surface,crack_front]
        ...
        padding = 3
        r_I = 20
        [ax,ay,az] = diamond(self.el, self.a0, [1,1,1], crack_surface, crack_front).cell.lengths()
        n = [2 * (int(np.ceil(((r_III + 2*cutoff)/ ax)))+padding)+1, 2 * (int(np.ceil((r_III + 2*cutoff)/ ay))+padding+1), 1]

        cryst = diamond(self.el, self.a0, n, crack_surface, crack_front, cb=self.cb, shift=np.array([0.0,0.25,0.0]),switch_sublattices=True)
        self.cb.set_sublattices(cryst,A,read_from_atoms=True)

        cryst, theta = build_func(cryst,nzlayer,self.el,self.a0,diamond,crack_surface,crack_front,cb=self.cb,shift=np.array([0.0,0.25,0.0]),switch_sublattices=True)
    ...
    def test_build_cubic_111_periodic_cell(self):
        ...
        self.build_3D_cluster(generate_3D_cubic_111,nzlayer,crack_surface,crack_front)

    def test_build_3D_kink_periodic_cell(self):
        nzlayer = 5
        crack_surface = np.array([1, 1, 1])
        crack_front = np.array([1, -1, 0])
        self.build_3D_cluster(generate_3D_structure,nzlayer,crack_surface,crack_front)
