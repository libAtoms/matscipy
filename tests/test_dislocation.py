...
class TestDislocation(matscipytest.MatSciPyTestCase):
    ...
    def test__validate_disloc_inputs(self):
        ...
        for structure in bulks.keys():
            ...
            alat_valid, struct = sd._validate_disloc_inputs(alat, symbol="Cu", axes=axes, crystalstructure=structure)
        ...
        for structure in bulks.keys():
            bulk = bulks[structure]
            alat = bulk.cell[0, 0]
            alat_valid, struct = sd._validate_disloc_inputs(bulk, symbol="Cu", axes=axes, crystalstructure=structure)
        ...
        # "a" validation
        self.assertRaises(TypeError, sd._validate_disloc_inputs, "Not a bulk or lattice constant", symbol="Cu", axes=axes, crystalstructure="fcc")
        
        # Crystalstructure validation
        self.assertRaises(ValueError, sd._validate_disloc_inputs, bulk_fcc, symbol="Cu", axes=axes, crystalstructure="made up crystalstructure")
        self.assertRaises(TypeError, sd._validate_disloc_inputs, bulk_fcc, symbol="Cu", axes=axes, crystalstructure=["not the ", "right datatype"])

        # Bulk structure validation
        self.assertWarns(UserWarning, sd._validate_disloc_inputs, bulk_fcc, symbol="Cu", axes=axes, crystalstructure="bcc")
        self.assertWarns(UserWarning, sd._validate_disloc_inputs, bulk_fcc, symbol="Cu", axes=axes, crystalstructure="diamond")
        self.assertWarns(UserWarning, sd._validate_disloc_inputs, bulk_bcc, symbol="Cu", axes=axes, crystalstructure="fcc")

        ats = bulk_fcc.copy()
        del ats[0]
        # FCC with vacancy is not bulk FCC!
        self.assertWarns(UserWarning, sd._validate_disloc_inputs, ats, symbol="Cu", axes=axes, crystalstructure="fcc")

        
        # Test w/ supercells of bulk
        print()
        sd._validate_disloc_inputs(bulk_fcc * (2, 2, 2), symbol="Cu", axes=axes, crystalstructure="fcc")

        # Test w/ more complex crystal
        nacl = build_bulk("NaCl", crystalstructure="zincblende", a=5.8, cubic=True)

        sd._validate_disloc_inputs(nacl, symbol="Cu", axes=axes, crystalstructure="diamond")
