...
def test_eam_read_write(datafile_directory):
    source, parameters, F, f, rep = read_eam(
        f"{datafile_directory}/Au_u3.eam", kind="eam"
    )
    ...
    for i, p in enumerate(parameters):
        ...
        if diff is None:
            assert p == parameters1[i]
        else:
            print(i, p, parameters1[i], diff, tol, diff < tol)
            assert diff < tol
    assert (F == F1).all()
    assert (f == f1).all()
    np.testing.assert_allclose(rep, rep1)


def test_eam_alloy_read_write(datafile_directory):
    source, parameters, F, f, rep = read_eam(
        f"{datafile_directory}/CuAgNi_Zhou.eam.alloy", kind="eam/alloy"
    )
    write_eam(
        source,
        parameters,
    ...
    for i in range(len(rep)):
        for j in range(len(rep)):
            if j < i:
                assert (rep[i, j, :] == rep1[i, j, :]).all()


def test_eam_fs_read_write(datafile_directory):
    source, parameters, F, f, rep = read_eam(
        f"{datafile_directory}/CuZr_mm.eam.fs", kind="eam/fs"
    )
...
@pytest.mark.skip("LP: Not sure why this test is skipped")
