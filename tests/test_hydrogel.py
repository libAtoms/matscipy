...
class TestLucyWeightFunction:
    ...
    def test_at_zero(self):
        ...
        rc = 5.0
        w0 = lucy.at_zero(rc)
        w_near_zero = lucy(np.array([1e-10]), rc)[0]
    ...
    def test_continuity_at_cutoff(self):
        ...
        w_at_rc = lucy(np.array([rc]), rc)[0]
        assert w_at_rc == 0.0

        # Should be zero beyond cutoff
        w_beyond = lucy(np.array([rc + 0.1]), rc)[0]
        assert w_beyond == 0.0
    ...
    def test_derivative_continuity(self):
        ...
        # Should be zero at cutoff
        dw_at_rc = lucy.derivative(np.array([rc - 1e-10]), rc)[0]
    ...
    def test_numerical_derivative(self):
        ...
        lucy = LucyWeightFunction()
        rc = 5.0
        r = np.linspace(0.1, rc - 0.1, 50)
        h = 1e-6

        # Numerical derivative
        dw_num = (lucy(r + h, rc) - lucy(r - h, rc)) / (2 * h)

        # Analytical derivative
        dw_ana = lucy.derivative(r, rc)
    ...
    def test_numerical_second_derivative(self):
        ...
        d2w_num = (lucy.derivative(r + h, rc) - lucy.derivative(r - h, rc)) / (2 * h)

        # Analytical second derivative
        d2w_ana = lucy.second_derivative(r, rc)
