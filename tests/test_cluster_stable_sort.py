...
class TestStableSortCluster(matscipytest.MatSciPyTestCase):
    ...
    def build_cluster(self,lattice,el,a0,rI,rIII,cutoff,crack_surface,crack_front,shift=np.array([0.0,0.0,0.0])):
        padding = 3
        [ax,ay,az] = lattice(el, a0, [1,1,1], crack_surface, crack_front).cell.lengths()
        n = [2 * (int(np.ceil(((rIII + 2*cutoff)/ ax)))+padding)+1, 2 * (int(np.ceil((rIII + 2*cutoff)/ ay))+padding+1), 1]
        print('N VALS', n)  
    ...
    def stable_sort_test(self,lattice,el,a0,crack_surface,crack_front,shift=np.array([0.0,0.0,0.0])):
        ...
        big_cluster = self.build_cluster(lattice,el,a0,r_I_vals[-1],r_III_vals[-1],cutoff,crack_surface,crack_front,shift=shift)
        ...
        x, y, z_big = big_cluster_pos[:, 0], big_cluster_pos[:, 1], big_cluster_pos[:, 2]
        cx, cy = sx/2, sy/2
        ...
        for i in range(len(r_I_vals)-1):
            cluster = self.build_cluster(lattice,el,a0,r_I_vals[i],r_III_vals[i],cutoff,crack_surface,crack_front,shift=shift)
            small_cluster_pos = cluster.positions
            #get radial small cluster positions (from x and y)
            sx, sy, sz = cluster.cell.diagonal()
            x, y, z_small = small_cluster_pos[:, 0], small_cluster_pos[:, 1], small_cluster_pos[:, 2]
            cx, cy = sx/2, sy/2
            ...
            self.assertArrayAlmostEqual(big_cluster_r[:len(cluster)],small_cluster_r[:],tol=1e-5)
            #print(big_cluster_theta[7],small_cluster_theta[7])
            self.assertArrayAlmostEqual(big_cluster_theta[:len(cluster)],small_cluster_theta[:],tol=1e-5)
            self.assertArrayAlmostEqual(z_small,z_big[:len(cluster)],tol=1e-5)
    ...
    def test_stable_sort_silicon(self):
        ...
        self.stable_sort_test(diamond,el,a0,crack_surface,crack_front,shift=np.array([0.0,0.25,0.0]))
