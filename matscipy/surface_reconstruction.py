...
class SurfaceReconstruction:
    def __init__(self,el,a0,calc,directions,surf_dir,lattice='diamond'):
        ...
        if lattice == 'diamond':
            ...
            self.cb = CubicCauchyBorn(self.el,self.a0,self.calc,lattice=Diamond)
    ...
    def map_surface(self,fmax=0.0001,layers=6):
        ...
        #build the bulk structure, and the slab structure which has vacuum in surface direction
        bulk = self.lattice(directions=self.directions, size=size, symbol=self.el, latticeconstant=self.a0, pbc=(1,1,1))
        cell = bulk.get_cell() ; cell[self.surf_dir,:] *=2 # vacuum along self.surf_dir axis (surface normal)
        slab = bulk.copy() ; slab.set_cell(cell,scale_atoms=False)
        
        #get the mask for the bottom surface and fix it
        pos_before = slab.get_positions()
        mask = pos_before[:,self.surf_dir]<30 #fix bottom atoms to stop lower surface reconstruction
        slab.set_constraint(FixAtoms(mask=mask))
        ase.io.write('initial_slab.xyz',slab)
        slab.calc = self.calc

        #run an optimisation to relax the surface
        opt_slab = PreconLBFGS(slab) ; opt_slab.run(fmax=fmax)
        ...
        if self.cb is not None:
            ...
            #different relaxations happen for each multilattice atom in each atomic layer near the surface
            self.cb.set_sublattices(bulk,self.A)
            #in a multilattice, there are 2 atomic layers per unit cell, so set the height to double the atomic layers
            total_layers = 2*height
            n_atoms_per_layer = int(len(slab)/total_layers)

            #get the indices of the atoms based on how close they are to the surface in order, for each sub lattice
            sorted_surf_indices_lattice1 = np.argsort(pos_before[self.cb.lattice1mask][:,self.surf_dir])
            sorted_surf_indices_lattice2 = np.argsort(pos_before[self.cb.lattice2mask][:,self.surf_dir])
            ...
            surf_layer_list_lattice1 = np.zeros([total_layers,int(n_atoms_per_layer/2)],dtype=int)
            surf_layer_list_lattice2 = np.zeros([total_layers,int(n_atoms_per_layer/2)],dtype=int)
            for i in range(total_layers):
                surf_layer_list_lattice1[i,:] = sorted_surf_indices_lattice1[i*int(n_atoms_per_layer/2):(i+1)*int(n_atoms_per_layer/2)]
                surf_layer_list_lattice2[i,:] = sorted_surf_indices_lattice2[i*int(n_atoms_per_layer/2):(i+1)*int(n_atoms_per_layer/2)]
            ...
            # the lattice 1 and lattice 2 atoms in these layers in the form [x,y,z] for each layer
        
            self.relaxation_array_lattice1 = np.zeros([layers,3])
            self.relaxation_array_lattice2 = np.zeros([layers,3])
            for layer in range(layers):
                pos_shifts_lattice1 = pos_shift[self.cb.lattice1mask][surf_layer_list_lattice1[total_layers-layer-1,:]]
                pos_shifts_lattice2 = pos_shift[self.cb.lattice2mask][surf_layer_list_lattice2[total_layers-layer-1,:]]

                self.relaxation_array_lattice1[layer,:] = pos_shifts_lattice1[0,:]
                self.relaxation_array_lattice2[layer,:] = pos_shifts_lattice2[0,:]
            ...
            for i in range(height):
                surf_layer_list[i,:] = sorted_surf_indices[i*n_atoms_per_layer:((i+1)*n_atoms_per_layer)]
            
            self.inter_surf_dist = np.abs(pos_before[surf_layer_list[1,:]][0,surf_dir] - pos_before[surf_layer_list[0,:]][0,surf_dir])
    ...
    def identify_layers(self,atoms,surface_coords,xlim=None,ylim=None,zlim=None,search_dir=-1,atoms_for_cb=None):
        ...
        for layer in range(self.layers):
            ...
            #we then shift down or up a layer each iteration, depending on search_dir
            if search_dir == -1:
                ...
                layer_mask = np.logical_and((pos[:,self.surf_dir]<=surface_coords[self.surf_dir]\
                    -(layer*self.inter_surf_dist)),(pos[:,self.surf_dir]\
                    >surface_coords[self.surf_dir]-((layer+1)*self.inter_surf_dist)))
            elif search_dir == 1:
                ...
                print('search_dir should be -1 for a downward surface, 1 for an upward surface!')
                raise ValueError
            layer_mask = layer_mask&lim_mask
            
            layer_mask_set[:,layer] = layer_mask #add the layer mask found here to the full set
        ...
        if self.eval_cb is None:
            ...
            #in layers, so need to refine the masks further to be the lattice1 and lattice2 atoms in each layer
            if atoms_for_cb is None:
                atoms_for_cb = atoms
            #note that to find the sublattices, need to use a version of the atoms structure with no free surface
            ...
            for layer in range(self.layers):
                layer_mask_set_lattice1[:,layer] = layer_mask_set[:,layer]&self.eval_cb.lattice1mask
                layer_mask_set_lattice2[:,layer] = layer_mask_set[:,layer]&self.eval_cb.lattice2mask
            return layer_mask_set_lattice1, layer_mask_set_lattice2

        

    def apply_surface_shift(self,atoms,surface_coords,cb=None,xlim=None,ylim=None,zlim=None,search_dir=-1,atoms_for_cb=None):
        #This finds the layers of atoms that need shifting and applies the surface shift accordingly.
        self.eval_cb = cb
        if self.eval_cb is not None:
            layer_mask_set_lattice1,layer_mask_set_lattice2 = self.identify_layers(atoms,surface_coords,xlim=xlim,ylim=ylim,zlim=zlim,search_dir=search_dir,atoms_for_cb=atoms_for_cb)
            for layer in range(self.layers):
                ...
                #if the surface is going upward, invert surf_dir component of applied shift
                lattice_1_shift[self.surf_dir] = (-search_dir)*lattice_1_shift[self.surf_dir]
                lattice_2_shift[self.surf_dir] = (-search_dir)*lattice_2_shift[self.surf_dir]
                atoms.positions[:, :][layer_mask_set_lattice1[:,layer]] += lattice_1_shift
                atoms.positions[:, :][layer_mask_set_lattice2[:,layer]] += lattice_2_shift
                #print('after',atoms.positions[:, :][layer_mask_set_lattice1[:,layer]])

        else:
            layer_mask_set = self.identify_layers(atoms,surface_coords,xlim=xlim,ylim=ylim,zlim=zlim,search_dir=search_dir)
            for layer in range(self.layers):
                shift = self.relaxation_array[layer,:]
                shift[self.surf_dir] = (-search_dir)*shift[self.surf_dir]
                atoms.positions[:,:][layer_mask_set[:,layer]] += (-search_dir)*self.relaxation_array      
