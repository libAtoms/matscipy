...
def triplet_list_py(first_neighbours):
    ...
    The trplet list accepts an first_neighbour array (generated by
    ...
    # compute the triple list by processing first_i
    diffs = np.diff(first_neighbours)
    for i in range(len(diffs)):
...
def first_triplets_py(ij_t):
    ...
    description
    """
    curr_neighbour_pair = 0
    first_triplets  = np.zeros(1, dtype=int)
    for i, curr in enumerate(ij_t):
        if curr_neighbour_pair != curr:
            curr_neighbour_pair = curr
            first_triplets = np.concatenate([first_triplets, [i]])
    first_triplets = np.concatenate([first_triplets, [ij_t.size]])
