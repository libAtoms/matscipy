...
class AbellTersoffBrenner(Calculator):
    ...
    def calculate(self, atoms, properties, system_changes):
        ...
        xi_ij = np.bincount(ij_t, weights=xi_ij_ik)
...
class KumagaiTersoff(AbellTersoffBrenner):
    def __init__(self, A, B, lambda_1, lambda_2, eta, delta, alpha, beta, c_1,
        ...
        super().__init__(
            ...
            f_c=lambda r: (0.0+(r <= R_1)) if (r >= R_2 or r <= R_1) else (1/2+(9/16)*np.cos(np.pi*(r - R_1)/(R_2 - R_1)) - (1/16)*np.cos(3*np.pi*(r-R_1)/(R_2-R_1))),
...
Kumagai_CompMaterSci_39_457_Si_py_var = {
    "__ref__":  "T. Kumagai, S. Izumi, S. Hara, and S. Sakai, \
            Comput. Mater. Sci. 39, 457 (2007)",  # TODO: cite
    "el":        "Si"        ,
    "A":         3281.5905   ,
    "B":         121.00047   ,
    "lambda_1":  3.2300135   ,
