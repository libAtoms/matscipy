...
class EAM(Calculator):
    ...
    def calculate(self, atoms, properties, system_changes):
        ...
        # Forces
        reverse = find_indices_of_reverse_pairs(i_n, j_n)
...
def find_indices_of_reverse_pairs(i_n, j_n):
    ...
    dummies2 = np.arange(i_n.size)[sorted_2]
    dummies1 = np.arange(i_n.size)[sorted_1]
    reverse  = np.empty(i_n.size, dtype=i_n.dtype)
    reverse[dummies1] = dummies2
    return reverse
