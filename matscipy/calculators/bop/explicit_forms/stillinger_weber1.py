...
def StillingerWeber1():
    ...
    dU2 = lambda r: A * epsilon * (sigma/np.power(r-a*sigma,2)*(1-B*np.power(sigma/r,p)) - B*p/r*np.power(sigma/r,p)) * np.exp(sigma/(r-a*sigma))
    ddU2 = lambda r: A * epsilon * (np.power(sigma/r,p)*np.exp(sigma/(r-a*sigma))*(2*B*sigma/np.power(r-a*sigma,3) + B*P*sigma/(r*np.power(r-a*sigma,2))+B*sigma**2/np.power(r-a*sigma,4)) + \
                     np.power(sigma/r,p)*np.exp(sigma/(r-a*sigma))*B*p/r*(1/r + p/r+sigma/np.power(r-a*sigma)) + \
                     np.exp(sigma/(r-a*sigma))*(-2*sigma/np.power(r-a*sigma)-sigma**2/np.power(r-a*sigma,4))
                     )
    ...
    hf = lambda rij, rik: epsilon * f(ab(rik)) * np.exp(gamma*sigma/(ab(rik)-a*sigma)) * np.exp(gamma*sigma/(ab(rij)-a*sigma))
    d1h = lambda rij, rik: -gamma * sigma / np.power(ab(rij)-a*sigma, 2) * hf(rij, rik)
    d2h = lambda rij, rik: -gamma * sigma / np.power(ab(rik)-a*sigma, 2) * hf(rij, rik)
    d11h = lambda rij, rik: 2 * gamma * sigma / np.power(r-a*sigma, 3) * hf(rij, rik) - gamma * sigma / np.power(r-a*sigma,2) * d1h(rij, rik)
    d12h = lambda rij, rik: - gamma * sigma / np.power(r-a*sigma, 2) * d2h(rij, rik)
    d22h = lambda rij, rik: 2 * gamma * sigma / np.power(r-a*sigma, 2) * hf(rij, rik) - gamma * sigma / np.power(r-a*sigma,2) * d2h(rij, rik) 

    F = lambda r, xi: f(r) * U2(r) + lambda1 * f(r) * b(xi)
    d1F = lambda r, xi: f(r) * dU2(r)
    d2F = lambda r, xi: lambda1 * f(r)
    d11F = lambda r, xi: ddU2(r)
    d12F = lambda r, xi: 0*r**0
    d22F = lambda r, xi: 0*r**0
