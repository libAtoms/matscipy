...
class EDIPTersoffBrenner(Calculator):
    implemented_properties = ['energy']#, 'stress', 'forces', "hessian"]
    default_parameters = {}
    name = 'ThreeBodyPotential'
    def __init__(self, g_a, g_o, g, f_c, xi_ij, b_ij, psi_ij_rep, psi_ij_att, psi_ij):
        ...
        self.b_ij = b_ij
        self.psi_ij = psi_ij
        self.psi_ij_rep = psi_ij_rep
        self.psi_ij_att = psi_ij_att
    ...
    def calculate(self, atoms, properties, system_changes):
        ...
        ij_t, ik_t, first_ij, i_n, j_n, abs_dr_n, dr_nc = triplet_list('ijdD', atoms=atoms, cutoff=self.cutoff)
        
        # mask results to match the triplet absolut distance lists for r_ij and r_ik 
        ...
        self.cos_theta_n = self.cos_theta(self.r_ij, self.r_ik, dr_nc[ij_t], dr_nc[ik_t])
        ...
        psi_ij_att = self.psi_ij_att(self.r_ij_pair, f_c_pair, b_ij)
        psi_ij_rep = self.psi_ij_rep(self.r_ij_pair, f_c_pair)

        # add up repulsive and attractive energies
        e_n =  np.sum(psi_ij_rep) + np.sum(psi_ij_att)

        epot = 1/2*e_n
        
        self.results = {'energy': epot }

class KumagaiTersoff(EDIPTersoffBrenner):
    def __init__(self, A, B, lambda_1, lambda_2, eta, delta, alpha, beta, c_1, c_2, c_3, c_4, c_5, h, R_1, R_2, __ref__=None, el=None):        
        ...
        super().__init__(
            ...
            b_ij = lambda xi_ij:  (1+xi_ij**eta)**(-delta) ,# * np.sign(1+xi_ij**eta),
